...
const ZIP_REGION = [
  ['44439','Mesopotamia',41.4594,-80.9427],['44062','Middlefield',41.4511,-81.0346],['44099','Windsor',41.5492,-80.9833],
  ['44402','Bristolville',41.3980,-80.8528],['44491','West Farmington',41.3700,-80.9637],['44450','North Bloomfield',41.4457,-80.8196],
  ['44046','Huntsburg',41.5415,-81.0803],['44080','Parkman',41.3684,-81.0578],['44076','Orwell',41.5281,-80.8148],
  ['44021','Burton',41.4433,-81.1445],['44417','Farmdale',41.4332,-80.6633],['44410','Cortland',41.3463,-80.7277],
  ['44444','Newton Falls',41.1756,-80.9777],['44231','Garrettsville',41.3030,-81.0705],['44482','Warren',41.3174,-80.7613],
  ['44486','Warren',41.3174,-80.7613],['44481','Warren',41.1766,-80.9025],['44430','Leavittsburg',41.2379,-80.9108],
  ['44234','Hiram',41.3259,-81.1523],['44266','Ravenna',41.1690,-81.1970],['44024','Chardon',41.5782,-81.1913],
  ['44240','Kent',41.1330,-81.3424],['44484','Warren',41.2355,-80.7498],['44485','Warren',41.2404,-80.8478],
  ['44470','Southington',41.3003,-80.9727],['44473','Vienna',41.2466,-80.6627],['44446','Niles',41.1895,-80.7475],
  ['44483','Warren',41.2636,-80.8170],['44428','Kinsman',41.4308,-80.5852],['44418','Fowler',41.3069,-80.6046],
  ['44403','Brookfield',41.2405,-80.5827],['44437','McDonald',41.1559,-80.7299],['44438','Masury',41.2397,-80.5307],
  ...
  ['44082','Pierpont',41.7619,-80.5675],['44047','Jefferson',41.7279,-80.7359],['44032','Dorset',41.6692,-80.6701],
...
function haversineMiles(lat1, lon1, lat2, lon2) { const r=Math.PI/180,a=Math.sin((lat2-lat1)*r/2)**2+Math.cos(lat1*r)*Math.cos(lat2*r)*Math.sin((lon2-lon1)*r/2)**2; return EARTH_RADIUS_MILES*2*Math.atan2(Math.sqrt(a),Math.sqrt(1-a)); }
function distanceForZip(zip, radius = Infinity) { const z=normalizeZip(zip),row=ZIP_REGION.find(x=>x[0]===z); if(!row)return null; const d=Math.round(haversineMiles(HOME_COORDS[0],HOME_COORDS[1],row[2],row[3])); return d<=Number(radius)?d:null; }
function eligibleZips(radius=30) { const r=Number(radius); return ZIP_REGION.map(([zip,city])=>({zip,city,distance:distanceForZip(zip)})).filter(x=>x.distance!==null&&x.distance<=r).sort((a,b)=>a.distance-b.distance); }
function isAllowedZip(zip,radius=30) { const z=normalizeZip(zip),d=distanceForZip(z); return !!z&&ZIP_SET.has(z)&&d!==null&&d<=Number(radius); }
