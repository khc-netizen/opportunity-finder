...
function querySet(interests, city, state) {
  ...
  const queries = [];
  for (const place of places) for (const term of terms.slice(0, 3)) {
...
async function search(query, budget) {
  ...
  try {
    ...
    const r = await fetch(u.href, { redirect: "follow", headers: { "User-Agent": "Opportunity-Finder/3.10-organic", Accept: "text/html,application/xhtml+xml" } });
...
async function fetchPage(url, budget) {
  ...
  budget.used++;
  try {
    const r = await fetch(url, { redirect: "follow", headers: { "User-Agent": "Opportunity-Finder/3.10-organic", Accept: "text/html,application/xhtml+xml" } });
    if (!r.ok) return null;
    const html = await r.text();
...
function makeGroup(page, interests, city, state) {
  ...
  return { title: page.title || host(page.url), name: page.title || host(page.url), url: page.url, link: page.url, description, location: `${city}, ${state}`, source: "organic-search", relevance: scoreCandidate({ text: `${page.title} ${page.description}`, url: page.url }, interests, city, state) };
}
...
export async function organicDiscover(interests, city, state, radius = 75) {
  ...
  return { ok: true, version: "3.10.0", build: "v3.10.0-organic-first", architecture: "organic organization discovery with explicit seed fallback", groups: groupResult.groups, events: eventResult.events, jobs: [], fetchBudget: { used: budget.used, limit: budget.limit }, coverage: { radius, organicGroups: groupResult.groups.length, organicEvents: eventResult.events.length, candidateCount: groupResult.candidateCount }, diagnostics: [...groupResult.diagnostics, ...eventResult.diagnostics], durationMs: Date.now() - started };
}
