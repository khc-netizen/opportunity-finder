...
async function run(request) { const p=params(request); const result=await discoverAll(p); const anchorDiagnostics=diagnoseKnownLocalSources(result); return release({ok:true,version:RELEASE,build:RELEASE_BUILD,architecture:'organization-first / organic-first / diagnostic local anchors / candidate-only hard ZIP gate / independent ZIP radii / configurable exclusions / ranked search evidence',...result,diagnostics:[...(result.diagnostics||[]),...anchorDiagnostics],coverage:{homeZip:HOME_ZIP,groupRadius:p.groupRadius,eventRadius:p.eventRadius,jobRadius:p.jobRadius,groupEligibleZipCount:eligibleZips(p.groupRadius).length,eventEligibleZipCount:eligibleZips(p.eventRadius).length,jobEligibleZipCount:eligibleZips(p.jobRadius).length,hardZipGate:true,prefetchFiltering:true,searchEvidence:true,independentRadii:true,configurableExclusions:true,diagnosticLocalAnchors:true}}); }
